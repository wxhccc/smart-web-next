<script lang="ts">
import { computed, defineComponent, mergeProps, h, ref, VNodeProps, Slot } from 'vue'
import { Form, message, Modal, FormInstance } from 'ant-design-vue'
...
export default defineComponent({
  ...
  setup(props, { emit, attrs, slots }) {
    ...
    const submitHandler = async () => {
      ...
      paramsHandler instanceof Function && (values = paramsHandler(values))
    ...
    const skipHandle = (fnOrRouteName?: SkipRouteOrFn) => {
      ...
      fnOrRouteName && (fnOrRouteName instanceof Function ? fnOrRouteName() : router.push({ name: fnOrRouteName }))
    }

    const modalProps = computed(() =>
      mergeProps(
        {
          class: 'common-fixed-modal simple-form-modal',
          centered: true,
          maskClosable: false,
          onCancel: closeModal
        },
    ...
    return () => {
      if (props.isModal) {
        return h(Modal, modalProps.value, { default: formRender, footer: submitBtnsNodes })
      }
      return formRender()
    }
  }
